...
export async function auditCommand(rootDir, options) {
    ...
    const written = [];
    if (options.report)
        written.push(...(await writeAllReports(rootDir, report)));
    if (options.markdown)
        written.push(await writeMarkdownReport(rootDir, report));
    if (options.html)
        written.push(await writeHtmlReport(rootDir, report));
    if (options.json)
        written.push(await writeJsonReport(rootDir, report));
    if (options.copy) {
