...
import {
  select,
  renderCategorizedTally,
  renderScoreBox,
  buildDoctorPrompt,
} from "../reporting/doctor.js";
...
export async function doctorCommand(rootDir: string): Promise<void> {
  ...
  let scanChoice: "full" | "changed" = "full";
  if (isRepo) {
    scanChoice = await select("Choose what to scan", [
      { title: "Full codebase", value: "full" as const },
      { title: "Changed files only", value: "changed" as const },
    ]);
  } else {
  ...
  const nextAction = await select("What would you like to do next?", [
    { title: "Copy prompt to clipboard", value: "copy" as const },
