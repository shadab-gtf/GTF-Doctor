...
export async function runSkeletonEngine(context: AuditContext): Promise<EngineReport> {
...
export function getComponentFiles(files: ProjectFile[]): ProjectFile[] {
  return files.filter(
    (file) =>
      /\.(tsx|jsx)$/.test(file.relativePath) &&
