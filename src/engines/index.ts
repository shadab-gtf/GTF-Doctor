...
export async function runEnabledEngines(context: AuditContext, config: ResolvedGtfConfig): Promise<EngineReport[]> {
  const runners: Array<Promise<EngineReport> | undefined> = [
  ...
  return Promise.all(runners.filter((runner): runner is Promise<EngineReport> => runner !== undefined));
}
