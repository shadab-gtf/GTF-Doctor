...
export async function writeMarkdownReport(rootDir: string, report: AuditReport): Promise<string> {
  const target = path.join(rootDir, REPORT_DIR, "audit-report.md");
  await writeTextFile(target, renderMarkdownReport(report));
  return target;
}

export async function writeHtmlReport(rootDir: string, report: AuditReport): Promise<string> {
  const target = path.join(rootDir, REPORT_DIR, "audit-report.html");
  await writeTextFile(target, renderHtmlReport(report));
  return target;
}

export async function writeJsonReport(rootDir: string, report: AuditReport): Promise<string> {
  const target = path.join(rootDir, REPORT_DIR, "audit-report.json");
  await writeTextFile(target, renderJsonReport(report));
  return target;
}

export async function writeAllReports(rootDir: string, report: AuditReport): Promise<string[]> {
